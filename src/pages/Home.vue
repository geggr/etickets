<template>
    <div class="landing-banner h-[500px]">
        <container class="flex flex-col justify-center items-center gap-y-8 h-full">
            ...
            <router-link to="/dashboard"
                class="bg-[#A32FFF] text-white font-display text-3xl rounded-md py-4 w-1/6 text-center">Entrar</router-link>
        </container>
    </div>
    ...
    <container class="mt-16">
        ...
        <ul class="flex flex-wrap gap-x-16 gap-y-16 mt-10">
                <event-vertical-card
                    v-for="(_, index) in numberOfCards"
                    :key="index"
                    title="Coldplay: Music of Spheres"
                    date="04/10/2023"/>
        </ul>
        <router-link to="/dashboard"
                class="mt-6 block bg-brand-secondary text-white font-display text-3xl rounded-md py-4 w-1/2 text-center mx-auto">Conhecer a Plataforma</router-link>
    </container>

    <div class="mt-10 bg-brand-secondary mb-10">
        <container class="landing-cta h-60 flex items-center">
            <h4 class="text-5xl uppercase font-bold text-white inline-block text-center">Cadastrou <br/> Trocou <br/> Aproveitou </h4>
        </container>
    </div>
